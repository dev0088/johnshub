...
import { getMonthName, getDatesFromWeek } from '../../../../utils/datetimeUtils';
import { getMonthlyResum, getWeeklyResum } from '../../../../utils/analyticsResum';

export default function LivePreview(props) {
  const { numbersPerStation } = props;
  const monthly = (numbersPerStation && numbersPerStation.monthly) || null;
  const weekly = (numbersPerStation && numbersPerStation.weekly) || null;
  
  var renderMonthlyItems = [];
  var renderWeeklyItems = [];
  const {monthlyResum, monthlyTotalRentCount, monthlyTopIndex} = getMonthlyResum(monthly);
  const {weeklyResum, weeklyTopIndex} = getWeeklyResum(weekly);
